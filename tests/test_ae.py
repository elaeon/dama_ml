...
class TestUnsupervicedModel(unittest.TestCase):
    ...
    def train(self, ae, model_params=None):
        with Data(name="tsne", dataset_path="/tmp", driver=HDF5(mode="w")) as dataset, \
                Data(name="test", dataset_path="/tmp/", driver=HDF5(mode="w")) as ds:
            batch_size = 12
    ...
    def test_parametric_tsne(self):
        ...
        with Data(name="test0", dataset_path="/tmp", driver=HDF5(mode="w")) as dataset:
            dataset.from_data(self.x)

        with PTsne.load(model_version="1", model_name="tsne", path="/tmp/") as ae,\
            Data(name="test0", dataset_path="/tmp", driver=HDF5(mode="r")) as dataset:

...
def predict(classif, path, label_column):
    ...
    df = pd.read_csv(path)
    dataset_p = df.drop([label_column], axis=1).as_matrix()
    data = dataset_p
...
def graph(dataset):
    ...
    e0 = df_e[df_e["target"] == 1].sample(frac=0.1, replace=True).drop(["target"], axis=1).head(2000).as_matrix()
    #e1 = df_e[df_e["target"] == 0].sample(frac=0.03, replace=True).drop(["target"], axis=1).as_matrix()
    c0 = df_c[df_c["target"] == 0].sample(frac=0.1, replace=True).drop(["target"], axis=1).head(2000).as_matrix()
...
#normal()
import pandas as pd
v,l = dataset.new_validation_dataset(pd.read_csv("/home/sc/test_data/numerai_datasets/numerai_tournament_data.csv"), pd.read_csv("/home/sc/test_data/numerai_datasets/example_predictions.csv"), pd.read_csv("/home/sc/predictions.csv"), "t_id")
dataset.valid_data = v
dataset.valid_labels = l
classif = ml.clf_e.LSTM(dataset, check_point_path=check_point_path, pprint=False, timesteps=7)
test_train(classif, train=True, num_steps=5)
